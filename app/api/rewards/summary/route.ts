...
import { getRewardSummaryByUserId } from '@/lib/rewards'
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 토큰의 userId로 누적 통계 조회 (KST 기준 이번 달 포함)
    const summary = await getRewardSummaryByUserId(session.userId as number)
    ...
    return NextResponse.json(
      { success: false, error: '보상 요약 조회 실패' },
      { status: 500 }
    )
  }
}
